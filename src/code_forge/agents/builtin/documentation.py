"""
Documentation agent for creating comprehensive documentation.
"""

from __future__ import annotations

from ..base import Agent, AgentConfig, AgentContext
from ..result import AgentResult


class DocumentationAgent(Agent):
    """Agent specialized in generating documentation.

    Creates clear, comprehensive documentation including:
    - Module and function docstrings
    - README files
    - API documentation
    - Architecture diagrams
    """

    def __init__(
        self,
        task: str,
        config: AgentConfig,
        context: AgentContext | None = None,
    ) -> None:
        """Initialize documentation agent.

        Args:
            task: The documentation task.
            config: Agent configuration.
            context: Execution context.
        """
        super().__init__(task, config, context)

    @property
    def agent_type(self) -> str:
        """Return agent type identifier."""
        return "documentation"

    async def execute(self) -> AgentResult:
        """Execute documentation task.

        This method is called by the executor with proper
        LLM and tool integration.

        Returns:
            AgentResult indicating must use AgentExecutor.
        """
        # Actual execution is handled by AgentExecutor
        # This is called as a fallback or for testing
        return AgentResult.fail(
            "DocumentationAgent must be executed via AgentExecutor"
        )
