"""
Tutorial agent for creating educational content.
"""

from __future__ import annotations

from ..base import Agent, AgentConfig, AgentContext
from ..result import AgentResult


class TutorialAgent(Agent):
    """Agent specialized in creating tutorials.

    Creates step-by-step educational content:
    - Tutorials with progressive complexity
    - Onboarding materials
    - Learning paths
    - Practice exercises
    """

    def __init__(
        self,
        task: str,
        config: AgentConfig,
        context: AgentContext | None = None,
    ) -> None:
        """Initialize tutorial agent.

        Args:
            task: The tutorial creation task.
            config: Agent configuration.
            context: Execution context.
        """
        super().__init__(task, config, context)

    @property
    def agent_type(self) -> str:
        """Return agent type identifier."""
        return "tutorial"

    async def execute(self) -> AgentResult:
        """Execute tutorial creation task.

        This method is called by the executor with proper
        LLM and tool integration.

        Returns:
            AgentResult indicating must use AgentExecutor.
        """
        # Actual execution is handled by AgentExecutor
        # This is called as a fallback or for testing
        return AgentResult.fail(
            "TutorialAgent must be executed via AgentExecutor"
        )
