...
- **CONV-004: Smart Project Type Detection**
  - `ProjectTypeDetector` detects 20 project types from marker files
  - `ProjectInfo` dataclass with type, name, version, frameworks, tools
  - `LanguageProfile` provides language-specific hints and best practices
  - 8 pre-defined profiles: Python, JavaScript, TypeScript, Rust, Go, Java, Ruby, PHP
  - Detects frameworks: Django, FastAPI, React, Vue, Next.js, Rails, etc.
  - Detects package managers: Poetry, npm, Yarn, pnpm, Cargo, Bundler
  - Detects test frameworks: pytest, jest, vitest, cargo test
  - Detects build tools: Webpack, Vite, CMake, Gradle, Maven
  - `generate_project_context()` creates prompt-ready context strings
  - Auto-injects project context into system prompt
  - 81 new unit tests for detection and profiles

### Improved
- System prompt now includes project-specific hints and commands
- LLM receives framework-aware best practices
